...
#ifndef _RTK_DESKTOP_BASIC_ROW_LAYOUT
#define _RTK_DESKTOP_BASIC_ROW_LAYOUT
...
/** A partial class for arranging components in a row.
 * auto_bbox() and reformat() should be provided by derived classes.
...
public:
	 ...
	 */
	basic_row_layout(size_type xcells=0);

	/** Destroy row layout. */
	virtual ~basic_row_layout();

	virtual box auto_bbox() const = 0;
	virtual component* find(const point& p) const;
	virtual box bbox() const;
	virtual void resize() const;
	virtual void reformat(const point& origin,const box& pbbox) = 0;
...
public:
	 ...
	 */
	basic_row_layout& cells(size_type xcells);
	 ...
	 * @return a reference to this
	 */
	basic_row_layout& add(component& c,size_type x=npos);
	 ...
	 * @return a reference to this
	 */
	basic_row_layout& xgap(int xgap);
	 ...
	 * @return a reference to this
	 */
	basic_row_layout& margin(const box& margin);
	 ...
	 * @return a reference to this
	 */
	basic_row_layout& margin(int margin);
};
